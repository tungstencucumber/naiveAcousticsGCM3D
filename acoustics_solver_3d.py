...
class NaiveAcousticsSolver3D:
    ...
    def __init__(self, x_size, y_size, z_size, cp, rho, target_time, recording_time_step,
    ...
    def forward(self):
        ...
        u_prev[
            ...
            self.source_z_start_point:self.source_z_end_point,
            3
        ] = np.min(self.cp) * np.min(self.cp) * np.min(self.rho)
        ...
        for i in range(self.number_of_records):
            ...
            for j in range(self.steps_per_record):
                ...
                u_next, inv_next = self.do_split_step(u_prev, Uz, Uz1, c_neg, c_pos, direction=2)
                u_next, inv_next = self.do_split_step(u_next, Uy, Uy1, c_neg, c_pos, direction=1)
                u_next, inv_next = self.do_split_step(u_next, Ux, Ux1, c_neg, c_pos, direction=0)

                # TODO: emitting and reflecting top border
                # form = self.source(self.T)
                # p0 = np.zeros(shape=(self.num_points_x, self.num_points_y))
                # p0[self.source_x_start_point:self.source_x_end_point, self.source_y_start_point:self.source_y_end_point] = form
                # u_next[:, :, -1, 3] = p0
                # u_next[:, :, -1, 2] = inv_next[:, :, 0, 1] + p0 / (self.cp[:, :, 0] * self.rho[:, :, 0])
